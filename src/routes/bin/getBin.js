...
// Given a piece ID, get the bin ID that the piece goes into.
const getBin = (req, res) => {
  const pieceId = req.body.pieceId

  console.log('getBin called with pieceId:', pieceId, 'type:', typeof pieceId)
  ...
  for (let rowId in binMappings) {
    ...
    // If the row contains the given piece ID, send back the bin number.
  ...
  console.log(`pieceId ${pieceId} not found in any bin`)
  res.status(404).json({ error: 'Piece not found in any bin' })
}
